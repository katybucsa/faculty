...
public class StudentRepositoryTest {
    ...
    public void findOne() {
        stRepo.save(s1);
        Student s=stRepo.findOne(1);
        assert(s.getNume()=="Alin");
        try{
            stRepo.findOne(null);
        }catch(IllegalArgumentException iae){ assert(iae.getMessage().equals("Id invalid!\n"));}
    }
    ...
    public void update() {
        ...
        try{
            stRepo.update(s4);
        }catch(ValidationException ve){ assert(true);}
        assert(stRepo.update(s1)==s1);
        assert(stRepo.update(s2)==null);
        assert(stRepo.size()==1);
    }
    ...
    public void delete() {
        ...
        stRepo.save(s1);
        assert(stRepo.delete(2)==null);
        assert(stRepo.delete(1)==s1);
    }
}