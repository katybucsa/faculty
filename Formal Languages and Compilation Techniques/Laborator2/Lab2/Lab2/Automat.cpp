...
public:
	...
	*/
	...
	/*void printMachineDetails() {
		...
		std::cout << "Multimea tranzitii:";
		for (std::string key : getKeys(state)) {
			std::vector<std::string> trans = state.at(key).getKeys(state.at(key).getTransitions());
			for (std::string letter : trans) {
				std::vector<State> states = state.at(key).getTransitions().at(letter);
				std::cout << "\n" << key << " with " << letter << " -> ";
				for (State s : states) {
