...
public class TemaRepositoryTest {
    ...
    public void findOne() {
        repo.save(t1);
        Optional<Tema> t = repo.findOne(1);
        assert (t.get().getDescriere() == "numere complexe");
        try {
            repo.findOne(null);
    ...
    public void update() {
        ...
        assertEquals(repo.update(t1).get(), t1);
        assert (!repo.update(t2).isPresent());
        assert (repo.size() == 1);
    }
    ...
    public void delete() {
        ...
        repo.save(t1);
        assert (!repo.delete(2).isPresent());
        assert (repo.delete(1).get() == t1);
    }
}