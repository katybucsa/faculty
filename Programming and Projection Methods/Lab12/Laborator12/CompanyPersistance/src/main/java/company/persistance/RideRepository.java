 ...
 /*   public RideRepository(Properties properties, RideValidator v) {
    ...
    public Ride findOne(Integer id) {
        ...
        try (PreparedStatement stat = conn.prepareStatement("select * from Ride where ride_id=?")) {
            ...
            logger.error(e);
            throw new RepositoryException("Ride couldn't be found!");
        }
    }
    ...
    public void save(Ride ride) {
        ...
        valid.validate(ride);
        Connection conn = dbConnection.getConnection();
        try (PreparedStatement stat = conn.prepareStatement("INSERT INTO Ride (destination,date,hour,places) VALUES (?,?,?,?)")) {
            stat.setString(1, ride.getDestination());
            stat.setString(2, ride.getDate());
            stat.setString(3, ride.getHour());
            stat.setString(4, ride.getPlaces());
            stat.executeUpdate();
        } catch (SQLException e) {
            logger.error(e);
    ...
    public void update(Ride c) {
        ...
        valid.validate(c);
        Connection conn = dbConnection.getConnection();
        try (PreparedStatement stat = conn.prepareStatement("UPDATE Ride SET places=?, destination=?, date=?, hour=? WHERE ride_id=?")) {
            stat.setString(1, c.getPlaces());
            stat.setString(2, c.getDestination());
            stat.setString(3, c.getDate());
            stat.setString(4, c.getHour());
