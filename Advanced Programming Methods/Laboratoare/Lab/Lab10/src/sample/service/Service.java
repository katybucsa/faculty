...
import sample.repository.NotaRepository;
import sample.repository.RepositoryInterface;
import sample.repository.TemaRepository;
...
public class Service implements Observable<AppEvent> {
    ...
    public int getCurrentWeek() {
        return new GregorianCalendar().get(Calendar.WEEK_OF_YEAR) - 39;
    }
    ...
    public Integer getCurrentHomework() {
        for (Tema t : findAllTeme())
            if (t.getDeadline() == getCurrentWeek())
                return t.getID();
        return null;
    }

    public Tema getTemaByDeadline(int deadline) {
        for (Tema tm : findAllTeme())
            if (tm.getDeadline() == deadline)
                return tm;
    ...
    public double addNota(int idS, int idT, double nota, LocalDateTime date, String feedback, Boolean flag) {
        Optional<Tema> t;
        Optional<Student> s;
        ...
        if (flag) {
            ...
            n = nota - 2.5 * depunctari;
            n1 = 10 - 2.5 * depunctari;
        }
        if (n < 1)
            n1 = n = 1;
        Nota grade=new Nota(idS, idT, n, date, feedback);
        Nota2 gr=new Nota2(idS,idT,findOneStudent(idS).get().getNume(),n,date,feedback);
        nRepo.save(grade);
        makeStudentFile(s.get().getNume(), idT, n, t.get().getDeadline(), feedback);
        notifyObservers(new AppEvent(Action.ADDN, gr));
        return n1;
        }
