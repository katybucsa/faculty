...
void testsRepository() {
	...
	std::map<std::string,Oferta> m = repo.getAll();
	assert(m[0] == o2);
	assert(m[1] == o1);
	repo.remove(o2);
	assert(repo.sizeElems() == 1);
...
void testsService() {
	...
	std::map<std::string,Oferta> wl = serv.allFromWishlist();
	assert(wl.size() == 1);
	serv.addRandom(4);
...
void testsFileRepository() {
	...
	try {
		auto i = frepo.find(Oferta("ds", "fwe", "fds", 54)); assert(false);
	...
	std::map<std::string,Oferta> v = frepo.getAll();
	assert(v.size() == 1);
