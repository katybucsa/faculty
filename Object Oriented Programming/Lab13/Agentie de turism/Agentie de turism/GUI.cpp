...
void OfertaGUI::uploadList(const std::vector<Oferta>& v) {
	listOfAll->clear();
	//listOfAll->setUserData()
	QListWidgetItem* item = new QListWidgetItem("Name\tDestination\t     Type\tPrice\n", listOfAll);
	for (const auto& o : v) {
		QListWidgetItem* item = new QListWidgetItem(QString::fromStdString(o.getName() + "\t    " + o.getDestination() + "\t     " + o.getType()) + "\t" + QString::number(o.getPrice()), listOfAll);
	}
