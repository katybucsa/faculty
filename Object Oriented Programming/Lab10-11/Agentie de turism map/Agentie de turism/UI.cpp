...
void UI::printOffers(const std::map<std::string,Oferta>& offers) {
	std::cout << std::endl;
	for (const auto& offer : offers)
		std::cout << offer.second;
...
void UI::uiFilterByDestination() {
	...
	std::cout << "Introduceti destinatie: ";
	std::getline(std::cin, destination);
	const std::map<std::string,Oferta> v = serv.filterByDestination(destination);
	if (v.size() == 0)
...
void UI::uiFilterByPrice() {
	float price;
	price = read<float>("Introduceti pret: ", "Pret invalid!\n\n");
	const std::map<std::string,Oferta> v = serv.filterByPrice(price);
	if (v.size() == 0)
...
void UI::uiSortByName() {
	const std::map<std::string,Oferta> v = serv.sortByName();
	printOffers(v);
}

/*void UI::uiSortByDestination() {
	const std::map<std::string,Oferta> v = serv.sortByDestination();
	printOffers(v);
}


void UI::uiSortByTypePrice() {
	const std::map<std::string,Oferta> v = serv.sortByTypePrice();
	printOffers(v);
}
*/
...
void UI::printWishlistSize() {
	const std::map<std::string,Oferta> wlist = serv.allFromWishlist();
...
void UI::executeCmd(int nr) {
	const std::map<std::string,Oferta> v = serv.getAl();
	const std::map<std::string,Oferta>& wlist = serv.allFromWishlist();
	switch (nr)
	{
	case 1:
